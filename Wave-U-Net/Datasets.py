...
# import musdb
# from scipy.io.wavfile import read as read_wav # doesn't handle 24 bit??
...
def getURMP(database_path):
    print("getURMP({0})".format(database_path))
        ...
        if os.path.isdir(database_path+"/"+folder):
            ...
            for filename in os.listdir(database_path+"/"+folder):
                if (filename.endswith(".wav") and not filename.startswith(".")):
                    wav_file_list.append(os.path.join(database_path+"/"+folder, filename))

            # Condition on the number of instruments mixed

            # if two instruments (2 instrument + mix)
            if len(wav_file_list) == 3:
                tmp = []

                for wav in wav_file_list:

                    # TODO for each .wav file, extract info as needed in Sample.py
                    # Warning: URMP has 24 bit wav, cant use scipy library

                    # handle mix file first
                    if "AuMix" in wav:
                        with sf.SoundFile(wav, "r") as f:
                            tmp = [Sample(wav, f.samplerate, f.channels, len(f) / f.samplerate)] + tmp # prepending the mix file
                    else:
                        with sf.SoundFile(wav, "r") as f:
                            tmp.append(Sample(wav, f.samplerate, f.channels, len(f) / f.samplerate))

                samples.append(tmp)

    subsets.append(samples[2:])
    subsets.append(samples[:2])
