...
class MultistreamCache():
    def __init__(self,
        ...
        print('initializing MultistreamCache')
        ...
        seed()
        print('finished initializing MultistreamCache')
    ...
    def update_cache_from_queue(self):
        ...
        while True:
            ...
            #TODO only have additional condition if we actually want to use all available data samples
            # everytime? and self.communication_queue.empty())
            if (self.communication_queue.empty()):
                # print('self.communication_queue.empty() == True')
                break
            ...
            # print('Loading new item into cache from data list starting with ' + str(self.worker_options["file_list"][0]))
            self.update_next_cache_item(self.communication_queue.get())
