...
def cfg():
    # Base configuration
    model_config = {
                    # "musdb_path" : "/home/daniel/Datasets/MUSDB18", # SET MUSDB PATH HERE, AND SET CCMIXTER PATH IN CCMixter.xml
                    "URMP_path" : "/home/leo/Documents/URMP", # SET MUSDB PATH HERE, AND SET CCMIXTER PATH IN CCMixter.xml
...
def dsd_100_experiment(model_config):
    ...
    if os.path.exists('dataset.pkl'):
        ...
        dsd_train, dsd_test = Datasets.getURMP(model_config["URMP_path"])
        # ccm = Datasets.getCCMixter("CCMixter.xml") # only needed for vocals

        print(len(dsd_train))

        # Pick 3 random songs for validation from URMP train set (this is always the same selection each time since we fix the random seed!)
        val_idx = np.random.choice(len(dsd_train), size=3, replace=False)
    ...
    if model_config["task"] == "voice":
        ...
        dataset["train_sup"] = dataset["train_sup"][:11]
        for subset in ["train_sup", "valid", "test"]:
            for i in range(len(dataset[subset])):
                ...
                dataset[subset][i] = tuple(dataset[subset][i])

    # Optimize in a +supervised fashion until validation loss worsens
    sup_model_path, sup_loss = optimise(dataset=dataset)
    print("Supervised training finished! Saved model at " + sup_model_path + ". Performance: " + str(sup_loss))
    Evaluate.produce_source_estimates(model_config, sup_model_path, model_config["URMP_path"], model_config["estimates_path"], "train")
