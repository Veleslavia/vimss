...
from musdb import Track
from musdb import Source
from musdb import Target
import collections
import yaml
...
def predict(track):
    ...
    print('entered predict track: ' + track.path)
    ...
    print('unsample predicted source audio and conver to stereo')
    ...
    print('evaluating using museval...')
    # Evaluate using museval
    scores = museval.eval_mus_track(
        track, estimates, output_dir="/mnt/disks/datasets/musdb18/eval", # SiSec should use longer win and hop parameters here to make evaluation more stable!
    )
...
def predict_track(model_config, sess, mix_audio, mix_sr, sep_input_shape, sep_output_shape, separator_sources, mix_context):
    ...
    print('finished loading mixture')
    ...
    print('finished preallocating source predictions')
    ...
    print('finished padding mixture')
...
def produce_source_estimates(model_config, load_model, musdb_path, output_path, subsets=None):
    ...
    with open('/home/leo/Wave-U-Net/mus.yaml', 'r') as f:
        setup = yaml.load(f)
    ...
    #    for wav in os.listdir(os.path.join(musdb_path, folder)):
    #        musdb_wav_tracks.append(wav)

    musdb_stem_path = '/mnt/disks/datasets/musdb'
    tracks = []
    subset=subsets
    subset_folder = os.path.join(musdb_stem_path, subset)
    actual_subset_folder = os.path.join(musdb_path, subset)
    ...
    for _, folders, files in os.walk(subset_folder):
        ...
        for track_name in sorted(files):
            ...
            if 'stem' in track_name and track_name.endswith('.mp4'):
                ...
                track = Track(
                    name=track_name,
                    path=os.path.join(actual_subset_folder, track_name.replace('.mp4', '_mix.wav')),
                    subset=subset,
                    #stem_id=setup['stem_ids']['mixture'],
                    stem_id=None,
                    is_wav=True
                )
                print(track)
                ...
                sources = {}
                print('add sources to track =========')
                for src, source_file in list(setup['sources'].items()):
                    # create source object
                    print(src, source_file)
                    abs_path = os.path.join(actual_subset_folder, track_name.replace('.mp4', '_'+source_file))
                    print(abs_path)
                    if os.path.exists(abs_path):
                        sources[src] = Source(
                            name=src,
                            path=abs_path,
                            #stem_id=setup['stem_ids'][src],
                            stem_id=None,
                            is_wav=True
                        )
                ...
                for name, target_srcs in list(setup['targets'].items()):
    ...
    print(tracks)
    mus.run(predict, tracks=tracks, estimates_dir=output_path, subsets=subsets)
